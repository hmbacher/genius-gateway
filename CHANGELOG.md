...
## Migration Service
The per-service legacy-config hooks introduced in v1.3.0 (one private migration method per settings service plus a shared `LegacyConfigMigration` helper) were consolidated into a central **Migration Service** ([Migrations docs](https://hmbacher.github.io/genius-gateway/setup/system/#migrations)). Migrations are now declarative records (`id`, `shouldRun`, `apply`, failure policy) registered with a runner that fires twice during boot — once after the FS is mounted but before services load their files, once after services have started. Existing v1.3 split migrations were ported in place; behaviour for upgrading users is unchanged.

- **State surfaced via REST + UI**: `GET /rest/migrations` returns every registered migration with one of four resolved states (`applied` / `pending` / `notApplicable` / `failed`) plus phase, order, failure policy and per-state metadata (e.g. `appliedAt` for applied, `attempts` + `lastError` for failed). A new **System → Migrations** admin page renders the same data as a single table with state icons. Orphan entries (persisted as applied/failed but no longer registered in the current firmware) surface with `registered: false` so on-disk history can be told apart from the current registry
- **Failure is per-migration configurable**: `retryNextBoot` (default — leave unmarked, retry on next boot), `skipAfterRetries` (give up after `maxAttempts` boots), or `abortBoot` (halt the boot loudly — for crucial migrations that must not leave the device half-migrated). All three v1.3 migrations use `retryNextBoot`. Admins can clear the failure history via `POST /rest/migrations/retry` (or the Retry button on the System page) so given-up migrations become eligible again on next reboot
- **Framework-side hook upstreamable cleanly**: `ESP32SvelteKit::setPreServiceHook(std::function<void()>)` was added — fires once inside `begin()` right after `ESPFS.begin(true)` and before any service `.begin()`. The hook is generic (knows nothing about migrations); consumers register what they want. This means the upstream HA integration in `lib/framework/HomeAssistant/HASettingsService.{h,cpp}` is now migration-free and matches the upstream `pr/ha-integration` shape — all gateway-specific migrations live in `src/migrations/GatewayMigrations.cpp` and never bleed into framework code
- **State persisted at `/config/migrations.json`** as a small JSON document listing applied IDs (with `appliedAt` firmware version) and any failure records. Migration IDs are stable and version-prefixed (`v1.3-...`, `v1.4-...`); they are never renamed, only added

## PDF Export — Smoke Detector Report

A new **Generate PDF Report** button on the Smoke Detectors page produces a printable audit document with an overview page and a per-detector page for each registered device. Native SVG embedding is used throughout (logo, QR codes, icons) — no rasterisation.

- **pdfmake loaded externally, not bundled**: the library (~1.9 MB including the Roboto VFS font) is staged from `node_modules` into `interface/static/pdf/` by a new `interface/scripts/stage-pdf-assets.mjs` prebuild step (wired to npm `prebuild` / `predev` lifecycle hooks). SvelteKit's `adapter-static` copies the files verbatim into the build output; `build_interface.py`'s `rglob("*.*")` walk picks them up automatically and registers them as HTTP routes. No firmware changes required. `interface/static/pdf/` is gitignored; upgrading pdfmake is a single `npm update pdfmake`
- **Cached `<script>`-tag loader** replaces the original `await import('pdfmake/…')` calls in `pdfReport.ts`. Under `bundleStrategy: 'single'`, dynamic imports are inlined at build time and defeat lazy loading entirely — the script-tag approach injects two `<script>` tags on first use and caches the resulting Promise so subsequent invocations within the same session are instant
- **`PdfReportDialog`** wraps `ProgressDialog` for the PDF generation flow, opened via `modals.open()`. Steps surfaced to the user: *Fetching report data* → *Loading PDF library* → *Building report layout* → *Rendering PDF*. On error a toast is shown; the dialog always closes on completion. Opening via `modals.open()` avoids the `onBeforeClose must be called inside a component rendered by Modals` runtime error that inline `<ProgressDialog>` usage triggers

## Build
- **Auto-generated version files removed from git**: `lib/framework/AppVersion.h` and `interface/src/lib/version.ts` are stamped with a UTC build timestamp on every build by `vite-plugin-app-version`, making every build dirty in git. Both are now gitignored and untracked. A new `scripts/ensure_app_version.py` pre-build script (registered first in `extra_scripts`) writes a stub `#define APP_VERSION_FULL "0.0.0-00000000.000000"` if the header is absent — covering the case where `build_interface.py` skips the npm build because `WWWData.h` is already up-to-date
