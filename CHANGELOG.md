...
- **Commit deferred to a background task**: `_postCommitTask` runs the FS write plus the ~420 HA-discovery MQTT messages off the HTTP server task. The commit response now returns in milliseconds instead of blocking 15-20 s and starving the WebSocket keepalive
