<script lang="ts">
	...
	let active = $derived(tunerState === 'waiting' || tunerState === 'synced' || tunerState === 'decoding');
	...
	>
		...
		>
			...
			<div class="flex flex-col items-center justify-center px-6 py-8 space-y-6">
				{#if tunerState === 'waiting'}
					...
					<div class="relative flex flex-col items-center justify-center gap-2 w-40 h-40">
						...
						></div>
						<!-- Microphone icon -->
						<Microphone class="h-14 w-14 text-primary animate-pulse" />
				...
				{:else if tunerState === 'synced'}
					...
					<div class="relative flex items-center justify-center w-40 h-40">
						...
						></div>
						<div
							class="absolute inset-4 rounded-full bg-success/10"
						></div>
				...
				{:else if tunerState === 'decoding'}
					...
					<div class="relative flex items-center justify-center w-40 h-40">
						...
						<div class="absolute inset-0 rounded-full border-2 border-primary/40 animate-ping"
							style="animation-duration: 2s"
						></div>
						<div class="absolute inset-4 rounded-full border-2 border-primary/30 animate-ping"
							style="animation-duration: 2s; animation-delay: 0.5s"
						></div>
						<div class="absolute inset-8 rounded-full border-2 border-primary/20 animate-ping"
							style="animation-duration: 2s; animation-delay: 1s"
						></div>
						<Loader class="h-16 w-16 text-primary animate-spin stroke-2" />
					</div>
			...
			<div class="flex justify-end gap-2 p-6 pt-4 border-t border-base-300">
				<button
					class="btn btn-sm {tunerState === 'success' ? 'btn-success' : tunerState === 'error' ? 'btn-error' : 'btn-primary'}"
					onclick={() => {
						stopDetection();
						modals.close();
